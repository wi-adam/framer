...
//! progress readout. Like the other viewport renderers, it accepts explicit
//! read-only inputs and a pane-owned mutable runtime so several Render panes can
...
use crate::app::{RenderSettings, render_job};
...
/// Read-only inputs shared by every Render pane in one app frame.
pub(super) struct RenderView<'a> {
    pub(super) target_id: u64,
    pub(super) model: &'a BuildingModel,
    pub(super) settings: RenderSettings,
    pub(super) gpu_compute_ok: bool,
    pub(super) gpu_target_format: Option<wgpu::TextureFormat>,
    pub(super) gpu_ray_query_ok: bool,
    pub(super) ray_query_enabled: bool,
}

/// Mutable progressive state owned by exactly one viewport pane.
#[derive(Default)]
pub(in crate::app) struct RenderPaneState {
    pub(in crate::app) cpu: render_job::RenderViewState,
    pub(in crate::app) gpu: path_render::GpuRenderState,
    pub(in crate::app) motion_cooldown: u32,
}
...
/// Draws one path-traced Render pane. Geometry, materials, and lighting come
/// from `framer-render`; the heavy work runs on a background thread
/// ([`crate::app::render_job`]) and refines progressively while the camera is still.
pub(super) fn draw_project_render(
    ui: &mut Ui,
    input: RenderView<'_>,
    camera: &mut View3dState,
    state: &mut RenderPaneState,
) {
    let RenderView {
        target_id,
        model,
    ...
    if panning {
        camera.pan(response.drag_delta(), drawing.height());
    }
    ...
    if response.hovered() {
        ...
        if telephoto {
            ...
            camera.dolly_by((-scroll_y * 0.0015).exp());
            dollying = true;
        }
    }
    ...
    if orbiting || panning || zooming || dollying {
        state.motion_cooldown = MOTION_COOLDOWN_FRAMES;
    } else {
        state.motion_cooldown = state.motion_cooldown.saturating_sub(1);
    }
    let moving = state.motion_cooldown > 0;
    ...
    let (samples, target, accumulating, renderer) = if let (true, Some(format)) =
        (gpu_compute_ok, gpu_target_format)
    {
        let backend =
            path_render::PathTraceBackend::from_config(gpu_ray_query_ok, ray_query_enabled);
        let prepared = path_render::paint_for_target(
            target_id,
            &mut state.gpu,
            &painter,
            drawing,
        ...
        (
            state.gpu.samples(),
            state.gpu.target_spp(),
            state.gpu.is_accumulating(),
            backend.label(),
        )
    } else {
        ...
        let key =
            path_render::accumulation_key(render_job::model_signature(model), &opts, width, height);

        state.cpu.update(&ctx, model, opts, width, height, key);

        if let Some(texture) = state.cpu.texture() {
        ...
        (
            state.cpu.samples(),
            state.cpu.target_spp(),
            state.cpu.is_accumulating(),
            "CPU",
        )
    };
