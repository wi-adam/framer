...
| `panels.rs` | Model tree, inspector, app header quick-access/actions menus, workflow command strip, status bar — the egui panel bodies. Command placement rules live in [command-surfaces.md](specs/command-surfaces.md). The ceiling inspector edits per-ceiling slope (pitch + low edge), converting a room region to a polygon on enable. |
